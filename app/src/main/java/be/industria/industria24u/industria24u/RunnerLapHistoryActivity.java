...
public class RunnerLapHistoryActivity extends AppCompatActivity implements OnChildClickListener {
    ...
    private void setupViews() {
        Toolbar toolbar = (Toolbar) findViewById(R.id.runner_laps_toolbar);
        ...
        NavigationView navigationView = (NavigationView) findViewById(R.id.runner_laps_nav);
    ...
    private void displayData(List<Lap> laps) {
        if (!laps.isEmpty()) {
            ...
            Toast.makeText(app.getApplicationContext(), "You haven't ran any laps yet! Go on then, what are you waiting for?", Toast.LENGTH_SHORT).show();
        }
    }
