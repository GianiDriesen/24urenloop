...
public class RunnerTeamsAddMemberActivity extends AppCompatActivity {
    ...
    private class GetMembers extends AsyncTask<Void, Void, List<Person>> {
        ...
        protected void onPostExecute(List<Person> persons) {
            if (persons.size()==1) {
                ...
                app.getEntityMapper().gettMapper().getTeamByPersonAndYear(person,year);
                new GetTeamPerson().execute();
            }
    ...
    private class GetTeamPerson extends AsyncTask<Void, Void, Team> {
        protected Team doInBackground(Void... voids) {
            Team team = new Team();
            while (!app.getEntityMapper().dataReady()) {
                if (isCancelled()) break;
    ...
    private void updateDatabase() {
        app.getEntityMapper().getTeamHasPersonMapper().createTeamHasPerson(person,thisTeam);
